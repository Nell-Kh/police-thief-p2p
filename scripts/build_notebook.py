...
CELLS = [
    ...
    md(
        "## 8. Conclusions\n\n"
        "1. **The pinch cop was unfixable by tuning** - a flat 0% capture surface across "
        "its whole parameter grid, caused by the parity dance: equal speeds, orthogonal "
        "moves, and a trap trigger that never fires on the diagonal.\n"
        "2. **The region cop converts everywhere**: 1900/1900 start pairs, mean ~8 steps "
        "of the 35 allowed, ~2 barriers of the 14 allowed. Minimizing the thief's safe "
        "region makes progress irreversible - barriers never expire.\n"
        "3. **The strategy is parameter-robust, not parameter-sensitive**: every "
        "reasonable `(MIN_SHRINK, ENDGAME)` cell scores identically, and the crippled "
        "control (barriers disabled) collapses to 0% - the *endgame exit-sealing "
        "mechanism* is load-bearing, the thresholds around it barely matter. The shipped "
        "`MIN_SHRINK = 3`, `ENDGAME = 4` are kept as conservative quota guards. The "
        "thief-side sweep is equally flat: no `TRAP_RISK_PENALTY` defends against region "
        "strangulation, so the thief keeps `3`, its best value against pursuit-style "
        "cops it will actually meet in the league.\n"
        "4. **Token budget is comfortable**: worst-case series utilization is a small "
        "fraction of the 200k cap, and the template fallback bounds the worst case at "
        "zero tokens.\n"
        "5. **Belief, not truth, in real matches**: these are perfect-information results; "
        "in league play the region cop receives the belief argmax. Phase 7's finding - "
        "argmax exactly on the true cell - is what makes these numbers transfer."
    ),
]
